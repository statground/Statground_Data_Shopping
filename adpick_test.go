...
func TestAdpickWorkflowGateAndDedicatedCollectionMode(t *testing.T) {
	...
	job := text[start:]
	for _, required := range []string{"vars.ADPICK_CATALOG_ENABLED != 'false'", "secrets.ADPICK_API", `ADPICK_COLLECT_ONLY: 'true'`, "github.event_name == 'schedule' && 'standard'", "vars.ADPICK_QUERY_PROFILE || 'standard'", "adpick_coverage_summary.py", "actions/upload-artifact@v4", "local:" + adpickRawTable, "local:" + adpickSnapshotTable, "local:" + adpickPublishedTable, "Collect verify and publish travel and services catalogs"} {
